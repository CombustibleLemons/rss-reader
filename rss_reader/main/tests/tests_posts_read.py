...
class PostsReadTests(APITestCase):
    ...
    def test_posts_unread(cls):
        """Test all posts are unread at first"""
        # API
        # response = cls.client.get("/feeds/{}/posts/read".format(cls.f1.id))
        # cls.assertEqual(response.status_code, 200)
        # cls.assertItemsEqual(response.data['posts'], [])

        prObj = cls.user.readPosts.get(feed=cls.f1)
        cls.assertItemsEqual(prObj.getUnreadPosts(), cls.f1.posts.all())
    ...
    def test_mark_post_read(cls):
        ...
        cls.assertEqual(len(prObj.getUnreadPosts())+1, oldUnread)